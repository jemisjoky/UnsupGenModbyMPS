...
class MPS:
    ...
    def __init__(
        ...
        self.max_bondim = 200
        bondims = np.asarray(
            [2 ** min(i, self.n - 1 - i, np.log2(init_bondim)) for i in range(self.n)],
            np.int16,
        )
        ...
        self.tensors = [
            torch.randn([self.bondims[i - 1], 2, self.bondims[i]]).type(
                self._tensortype
            )
        ...
        self.merged_tensor = (
            self._tensortype()
        )  # self.merged_tensor will be on cpu and gid (in merge_bond() and rebuild_bond() for SVD), self.merged_tensor_BC will be the broadcasted version on each GPU
        self.tensor_bc = []
        self.merged_tensor_BC = (
            []
        )  # broadcast version of merged_tensor, will be put on each GPU.
        ...
        """
        self.gradients = (
            None  # storing gradients in each gpu, will be used in gradient_descent()
        )
        ...
        sys.stdout.write("left_canonical ")
        sys.stdout.flush()
        t1 = time.time()
        self.left_canonical()
        sys.stdout.write("%.2f s.\n" % (time.time() - t1))
        sys.stdout.write("Initiate NLL ")
        sys.stdout.flush()
        t1 = time.time()
        self.recompute_psi()
        ...
        print(
            "#%d nll=%.3f\tnll_test=%.3f\tmax_bondim=%d\t%.2f Sec."
            % (self.loop, self.nll, self.nll_test, max(self.bondims), time.time() - t1)
        )
        self.fname = fname + ".dat"
        self.nll_history = np.array(
            [(self.loop, self.nll, self.nll_test)],
            dtype=[("loop", "i"), ("train", "d"), ("test", "d")],
        )
        with open(self.fname, "a") as fp:
            print(
                "# nll_trn nll_tst max(bdm)\tduration\n%d %.3f %.3f %d"
                % (self.loop, self.nll, self.nll_test, max(self.bondims)),
                file=fp,
            )
    ...
    # {{{def _build_idx(self):
    def _build_idx(self):
        print("Building the 2-site index:", end="", flush=True)
        tt = time.time()
        ...
        for bond in range(self.n - 1):
            for i in range(2):
                for j in range(2):
                    self.idx_2sites_cpu[bond, i, j, :] = (
                        (self.data_cpu[:, bond] == i)
                        * (self.data_cpu[:, bond + 1] == j)
                    ).cpu()
    ...
    # {{{def distribute(self):
    def distribute(self):
        self.data = torch.cuda.comm.scatter(
            self.data_cpu, self._gpu_ids
        )  # declare data as long tensor for indexing purpuse, then broadcast it to all gpus
        ...
        if not hasattr(self, "idx_2sites_cpu"):
            self._build_idx()
        self.idx_2sites = torch.cuda.comm.scatter(
            self.idx_2sites_cpu, self._gpu_ids, dim=3
        )
        if self.m_test > 0:
            self.data_test = torch.cuda.comm.scatter(
                self.data_test_cpu, self._gpu_ids
            )  # declare data as long tensor for indexing purpuse, then broadcast it to all gpus
        ...
        for gidx in range(self._ngpu):
            print(
                "On GPU #%d, training data shape" % self._gpu_ids[gidx],
                tuple(self.data[gidx].shape),
                end=", ",
            )
            if self.m_test > 0:
                ...
                print("no test data")
        self.psis = (
            torch.ones(self._ngpu, self.m_gpu).type(self._tensortype).pin_memory()
        )
        ...
        if self.m_test > 0:
            self.psis_test = (
                torch.ones(self._ngpu, self.m_test_gpu)
                .type(self._tensortype)
                .pin_memory()
            )
    ...
    def merge_bond(self):
        ...
        self.merged_tensor = torch.mm(
            self.tensors[self.current_bond].cuda(self.main_gpuid).view(-1, d),
            self.tensors[self.current_bond + 1].cuda(self.main_gpuid).view(d, -1),
        ).view(
            self.bondims[self.current_bond - 1],
            2,
            2,
            self.bondims[self.current_bond + 1],
        )
    ...
    def train(self):
        ...
        print(
            "Batch head=%d, init cumulants..." % (self.batch_head), end="", flush=True
        )
        self.init_cumulants()
        print("%.2f Sec." % (time.time() - t1))
        self.going_right = False
        for bond in np.concatenate(
            (np.arange(self.n - 2, 0, -1), np.arange(0, self.n - 2))
        ).astype(np.uint16):
        ...
        sys.stdout.write("\r                                                       \r")
        print(
            "#%d batch_head=%d\tnll=%.3f\tnll_test=%.3f\tmax_bondim=%d\t%.2f Sec., SVD %.2f, other %.2f"
            % (
                self.loop,
                self.batch_head,
                nll_new,
    ...
    def _worker_init_cumulants(self, gidx):
        ...
        states = self.data[gidx][
            self.batch_head : self.batch_head + self.batch_size_gpu, self.current_bond
        ]
        ...
        while chunk_begin < self.batch_size_gpu:
            ...
            self.cumulants[self.current_bond + 1][gidx, chunk_begin:chunk_end, :, :] = (
                self.cumulants[self.current_bond][
                    gidx, chunk_begin:chunk_end, :, :
                ].cuda(gpuid)
                @ self.tensor_bc[gidx][:, state_loc, :].permute(1, 0, 2)
            ).cpu()
            self.cumulants[self.current_bond + 1][
                gidx, chunk_begin:chunk_end, :, :
            ] /= max(
                self.cumulants[self.current_bond + 1][gidx, chunk_begin:chunk_end]
                .abs()
    ...
    def init_cumulants(self):
        ...
        self.cumulants.append(
            torch.ones(self._ngpu, self.batch_size_gpu, 1, 1)
            .type(self._tensortype)
            .pin_memory()
        )
        for site in range(
            self.n - 2
        ):  # n-2 for two-site update, notice that this would be n-1 for single-site update
            self.current_bond = site
            self.cumulants.append(
                torch.ones(self._ngpu, self.batch_size_gpu, 1, self.bondims[site])
                .type(self._tensortype)
                .pin_memory()
            )
            self.tensor_bc = torch.cuda.comm.broadcast(
                self.tensors[site], self._gpu_ids
            )
    ...
    def _worker_update_cumulants(self, gidx):
        ...
        while chunk_begin < self.batch_size_gpu:
            if self.going_right:
                newcumu = self.cumulants[self.current_bond][
                    gidx, chunk_begin:chunk_end
                ].cuda(gpuid) @ (
                    self.tensor_bc[gidx][:, states_loc[chunk_begin:chunk_end]].permute(
                        1, 0, 2
                    )
                )
            else:
                newcumu = self.tensor_bc[gidx][
                    :, states_loc[chunk_begin:chunk_end]
                ].permute(1, 0, 2) @ self.cumulants[self.current_bond + 1][
                    gidx, chunk_begin:chunk_end
                ].cuda(
                    gpuid
                )
    ...
    def update_cumulants(self):
        ...
        if self.going_right:
            self.tensor_bc = torch.cuda.comm.broadcast(
                self.tensors[self.current_bond], self._gpu_ids
            )
            self.cumulants[self.current_bond + 1] = self._tensortype(
                self._ngpu, self.batch_size_gpu, 1, self.bondims[self.current_bond]
            ).pin_memory()
        else:
            self.tensor_bc = torch.cuda.comm.broadcast(
                self.tensors[self.current_bond + 1], self._gpu_ids
            )
            self.cumulants[self.current_bond] = self._tensortype(
                self._ngpu, self.batch_size_gpu, self.bondims[self.current_bond], 1
            ).pin_memory()
    ...
    def _worker_gradient_descent(self, gidx):
        ...
        while chunk_begin < self.batch_size_gpu:
            ...
            cumu2 = self.cumulants[self.current_bond + 1][
                gidx, chunk_begin:chunk_end, :, :
            ].cuda(gpuid)
            converter = (
                torch.arange(chunk_end - chunk_begin).type(torch.LongTensor).cuda(gpuid)
            )
            for i, j in [(i, j) for i in range(2) for j in range(2)]:
                ...
                psis = (
                    lvecs
                    @ self.merged_tensor_BC[gidx][:, i, j, :].repeat(idx.numel(), 1, 1)
                    @ rvecs
                )
                psis += (
                    self.epsi
                )  ### this is to resolve the zero psis issue, for the float-storage
                lvecs /= psis
                torch.addbmm(
                    self.gradients[gidx][:, i, j, :],
                    lvecs.permute(0, 2, 1),
                    rvecs.permute(0, 2, 1),
    ...
    def gradient_descent(self):
        self.merged_tensor_BC = torch.cuda.comm.broadcast(
            self.merged_tensor, self._gpu_ids
        )
        self.gradients = [
            torch.zeros(
                self.bondims[self.current_bond - 1],
        ...
        gradients = (
            torch.cuda.comm.reduce_add(self.gradients, self.main_gpuid).cuda(
                self.main_gpuid
            )
            / self.batch_size
            - self.merged_tensor
    ...
    def _worker_recompute_psi(self, gidx):
        ...
        while chunk_begin < self.m_gpu:
            ...
            psis = (
                torch.ones(chunk_end - chunk_begin, 1, 1)
                .type(self._tensortype)
                .cuda(gpuid)
            )
            for i in range(self.n):
                psis @= self.tensors[i].cuda(gpuid)[:, state[:, i], :].permute(1, 0, 2)
            self.psis[gidx, chunk_begin:chunk_end] = psis.view(
                -1,
            )
            chunk_begin = chunk_end
    ...
    def _worker_recompute_psi_test(self, gidx):
        ...
        while chunk_begin < self.m_test_gpu:
            ...
            for i in range(self.n):
                psis @= self.tensors[i].cuda(gpuid)[:, state[:, i], :].permute(1, 0, 2)
            self.psis_test[gidx, chunk_begin:chunk_end] = psis.view(
                -1,
            )
            chunk_begin = chunk_end
    ...
    def _log(self, duration=0):
        self.nll_history = np.append(
            self.nll_history,
            np.array(
                (self.loop, self.nll, self.nll_test), dtype=self.nll_history.dtype
            ),
        )
        with open(self.fname, "a") as fout:
            print(
                "%d %.3f %.3f %d\t\t%.1f"
                % (self.loop, self.nll, self.nll_test, max(self.bondims), duration),
                file=fout,
                flush=True,
            )
        with open("hyper.txt", "a") as fp:
    ...
    def generate_sample(self, stat=None, givn_mask=None, gid=0):
        ...
        if givn_mask[p_unnorm] == False:
            ...
            if p_unnorm > targ_p_unnorm:
                ...
                while p_unnorm > targ_p_unnorm:
                    ...
                    p_unnorm -= 1
                    print(
                        "Canonicalization: unnormalized site = #%d" % p_unnorm,
                        end="\r",
                        flush=True,
                    )
        ...
        mid_mat_fromleft = self.tensors[p_given_leftmost][
            :, state[p_given_leftmost]
        ].cuda(gpu)
        ...
        if p_intermid_ungiven_leftmost < p_given_rightmost + 1:
            ...
            while p > p_intermid_ungiven_leftmost:
                if givn_mask[p]:
                    ...
                    right_vecs[p] = right_vecs[p].view(tens.shape[0], -1) @ (
                        tens.view(tens.shape[0], -1).t()
                    )
    ...
    def save(self, fsave_name=None):
        ...
        with open("mps.pickle", "wb") as fsave:
            tsave = time.time()
            sys.stdout.write("saving mps object to %s ..." % dit)
            sys.stdout.flush()
        ...
        if self.m_test > 0:
            np.savez_compressed(
                "../data.npz",
                train=self.data_cpu.numpy().astype(np.uint8),
                test=self.data_test_cpu.numpy().astype(np.uint8),
            )
        else:
            np.savez_compressed(
                "../data.npz", train=self.data_cpu.numpy().astype(np.uint8), test=None
            )
        os.chdir(orig_dir)
